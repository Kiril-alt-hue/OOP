...
class RationalListIterator:
    def __init__(self, data):
        # лямда функція для сортування по знаменику/чисельнику
        self.sorted_data = sorted(data, key=lambda x: (-x.d, -x.n))
        self.index = 0
    ...
    def __next__(self):
        if self.index < len(self.sorted_data):
            result = self.sorted_data[self.index]
            self.index += 1
            return result
        else:
            raise StopIteration
...
def process_file(input_filename, output_filename):
    ...
    with open(output_filename, 'w', encoding="utf-8") as file:
        ...
        file.write("Елементи списку (відсортовані за спаданням знаменника, потім чисельника):\n")
        for num in rational_list:
            file.write(f"{num}\n")
...
if __name__ == "__main__":
    ...
    output_filename = "output_6.3.1.txt"
